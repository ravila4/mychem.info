...
def load_source(url):
    ...
    for x in links:
        ...
        with open(temp_file,"w") as _file:
            _file.write(data)        
        _f = gzip.open(temp_file,'r')        
        xmltodict.parse(_f,item_depth=2,item_callback=handle)  #parse the xml file to dictionary   
...
def restructure_dict(dictionary):
    ...
    for key,value in dictionary.iteritems():
        if key == "PC-Compound_id":
            for cnt in value:
                for m,n in value[cnt].iteritems():
                    for x,y in n.iteritems():
                        d["cid"] = y                        
            ...
            for cnt in value:
                for x,y in value[cnt].iteritems():
